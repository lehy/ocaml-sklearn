...
let%expect_test "RocCurveDisplay" =
  ...
  let pred = .array [0.1 0.4 0.35 0.8] np in  
  let fpr, tpr, thresholds = .roc_curve ~y pred metrics in  
  let roc_auc = .auc ~fpr tpr metrics in  
  let display = .rocCurveDisplay ~fpr:fpr ~tpr:tpr ~roc_auc:roc_auc ~estimator_name:'example estimator' metrics in  
  print_ndarray @@ .plot display # doctest: +SKIP;  
  [%expect {|
  |}]
...
let%expect_test "accuracy_score" =
  ...
  print_ndarray @@ accuracy_score ~y_true y_pred ();  
  [%expect {|
      0.5      
  |}]
  print_ndarray @@ accuracy_score ~y_true y_pred ~normalize:false ();  
  [%expect {|
      2      
  |}]
...
let%expect_test "accuracy_score" =
  let open Sklearn.Metrics in
  print_ndarray @@ accuracy_score(.array (matrixi [|[|0; 1|]; [|1; 1|]|])) np.ones((2 2)) np;  
  [%expect {|
  |}]
...
let%expect_test "auc" =
  ...
  let pred = .array [0.1 0.4 0.35 0.8] np in  
  let fpr, tpr, thresholds = .roc_curve ~y pred ~pos_label:2 metrics in  
  print_ndarray @@ .auc ~fpr tpr metrics;  
  [%expect {|
      0.75      
  |}]
...
let%expect_test "average_precision_score" =
  ...
  print_ndarray @@ average_precision_score ~y_true y_scores ();  
  [%expect {|
      0.83...      
  |}]
...
let%expect_test "balanced_accuracy_score" =
  ...
  print_ndarray @@ balanced_accuracy_score ~y_true y_pred ();  
  [%expect {|
  |}]
...
let%expect_test "brier_score_loss" =
  let open Sklearn.Metrics in
  let y_true = .array (vectori [|0; 1; 1; 0|]) np in  
  let y_true_categorical = .array ["spam" "ham" "ham" "spam"] np in  
  let y_prob = .array [0.1 0.9 0.8 0.3] np in  
  print_ndarray @@ brier_score_loss ~y_true y_prob ();  
  [%expect {|
      0.037...      
  |}]
  print_ndarray @@ brier_score_loss ~y_true 1-y_prob ~pos_label:0 ();  
  [%expect {|
      0.037...      
  |}]
  print_ndarray @@ brier_score_loss ~y_true_categorical y_prob ~pos_label:"ham" ();  
  [%expect {|
      0.037...      
  |}]
  print_ndarray @@ brier_score_loss ~y_true .array y_prob () > 0.5 np;  
  [%expect {|
      0.0      
  |}]
...
let%expect_test "classification_report" =
  ...
  let target_names = ['class 0', 'class 1', 'class 2'] in  
  print_ndarray @@ print(classification_report ~y_true y_pred ~target_names:target_names ());  
  [%expect {|
                    precision    recall  f1-score   support      
      <BLANKLINE>      
           class 0       0.50      1.00      0.67         1      
  ...
  print_ndarray @@ print(classification_report(y_true, y_pred, labels=(vectori [|1; 2; 3|])));  
  [%expect {|
                    precision    recall  f1-score   support      
      <BLANKLINE>      
                 1       1.00      0.67      0.80         3      
...
let%expect_test "confusion_matrix" =
  ...
  print_ndarray @@ confusion_matrix ~y_true y_pred ~labels:["ant" "bird" "cat"] ();  
  [%expect {|
      array([[2, 0, 0],      
             [0, 0, 1],      
             [1, 0, 2]])      
...
let%expect_test "confusion_matrix" =
  let open Sklearn.Metrics in
  let tn, fp, fn, tp = confusion_matrix((vectori [|0; 1; 0; 1|]), (vectori [|1; 1; 1; 0|])).ravel () in  
  print_ndarray @@ (tn, fp, fn, tp);  
  [%expect {|
  |}]
