...
let%expect_test "AdaBoostClassifier" =
  ...
  [%expect {|
      array([0.28..., 0.42..., 0.14..., 0.16...])      
  |}]
  print_ndarray @@ AdaBoostClassifier.predict (matrixi [|[|0; 0; 0; 0|]|]) clf;  
  [%expect {|
      array([1])      
  |}]
  print_ndarray @@ AdaBoostClassifier.score ~x y clf;  
  [%expect {|
  |}]
...
let%expect_test "AdaBoostRegressor" =
  let open Sklearn.Ensemble in
  let x, y = make_regression ~n_features:4 ~n_informative:2 ~random_state:(`Int 0) ~shuffle:false () in  
  ...
  [%expect {|
      array([0.2788..., 0.7109..., 0.0065..., 0.0036...])      
  |}]
  print_ndarray @@ AdaBoostRegressor.predict (matrixi [|[|0; 0; 0; 0|]|]) regr;  
  [%expect {|
      array([4.7972...])      
  |}]
  print_ndarray @@ AdaBoostRegressor.score ~x y regr;  
  [%expect {|
      0.9771...      
  |}]
...
let%expect_test "BaggingRegressor" =
  let open Sklearn.Ensemble in
  let x, y = make_regression ~n_samples:100 ~n_features:4 ~n_informative:2 ~n_targets:1 ~random_state:(`Int 0) ~shuffle:false () in  
