...
)); ("transformer_weights", transformer_weights); ("verbose", Wrap_utils.Option.map verbose Py.Bool.of_bool); ("transformer_list", Some(transformer_list |> (fun ml -> Py.List.of_list_map (fun (ml_0, ml_1) -> Py.Tuple.of_list [(Py.String.of_string ml_0); (Wrap_utils.id ml_1)]) ml)))])

                  let fit ?y ?fit_params ~x self =
                     Py.Module.get_function_with_keywords self "fit"
                       [||]
                       (List.rev_append (Wrap_utils.keyword_args [("y", Wrap_utils.Option.map y Ndarray.to_pyobject); ("X", Some(x |> (function
...
let inverse_transform ?x ?y self =
   Py.Module.get_function_with_keywords self "inverse_transform"
     [||]
     (Wrap_utils.keyword_args [("X", Wrap_utils.Option.map x Ndarray.to_pyobject); ("y", Wrap_utils.Option.map y Ndarray.to_pyobject)])
     |> Ndarray.of_pyobject
let predict ?predict_params ~x self =
   Py.Module.get_function_with_keywords self "predict"
     [||]
     (List.rev_append (Wrap_utils.keyword_args [("X", Some(x |> Ndarray.to_pyobject))]) (match predict_params with None -> [] | Some x -> x))
     |> Ndarray.of_pyobject
...
let score ?y ?sample_weight ~x self =
   Py.Module.get_function_with_keywords self "score"
     [||]
     (Wrap_utils.keyword_args [("y", Wrap_utils.Option.map y Ndarray.to_pyobject); ("sample_weight", Wrap_utils.Option.map sample_weight Ndarray.to_pyobject); ("X", Some(x |> Ndarray.to_pyobject))])
     |> Py.Float.to_float
...
let named_steps self =
  match Py.Object.get_attr_string self "named_steps" with
| None -> raise (Wrap_utils.Attribute_not_found "named_steps")
