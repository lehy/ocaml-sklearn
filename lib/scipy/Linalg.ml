...
let dgegv ?kwds args =
   Py.Module.get_function_with_keywords __wrap_namespace "dgegv"
     ...
     (match kwds with None -> [] | Some x -> x)
                  ...
                  let find_best_lapack_type ?arrays ?dtype () =
                     Py.Module.get_function_with_keywords __wrap_namespace "find_best_lapack_type"
                       [||]
                       (Wrap_utils.keyword_args [("arrays", Wrap_utils.Option.map arrays (fun ml -> Py.List.of_list_map Np.Obj.to_pyobject ml)); ("dtype", Wrap_utils.Option.map dtype (function
| `S x -> Py.String.of_string x
| `Dtype x -> Np.Dtype.to_pyobject x
...
module Linalg_version = struct
let () = Wrap_utils.init ();;
let __wrap_namespace = Py.import "scipy.linalg.linalg_version"
...
| `T_numpy_random_mtrand_RandomState_instance x -> Wrap_utils.id x
| `I x -> Py.Int.of_int x
