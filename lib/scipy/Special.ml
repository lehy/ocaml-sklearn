...
module Cython_special = struct
let () = Wrap_utils.init ();;
let __wrap_namespace = Py.import "scipy.special.cython_special"

let get_py name = Py.Module.get __wrap_namespace name
let bdtr ~x0 ~x1 ~x2 () =
   Py.Module.get_function_with_keywords __wrap_namespace "bdtr"
     [||]
     (Wrap_utils.keyword_args [("x0", Some(x0 )); ("x1", Some(x1 )); ("x2", Some(x2 ))])
...
let dawsn x0 =
   Py.Module.get_function_with_keywords __wrap_namespace "dawsn"
     [||]
     (Wrap_utils.keyword_args [("x0", Some(x0 ))])
...
let spherical_in ?derivative ~n ~z () =
   Py.Module.get_function_with_keywords __wrap_namespace "spherical_in"
     [||]
     (Wrap_utils.keyword_args [("derivative", derivative); ("n", Some(n )); ("z", Some(z ))])
...
let h1vp ?n ~v ~z () =
   Py.Module.get_function_with_keywords __wrap_namespace "h1vp"
     [||]
     (Wrap_utils.keyword_args [("n", Wrap_utils.Option.map n Py.Int.of_int); ("v", Some(v |> Np.Obj.to_pyobject)); ("z", Some(z |> Np.Obj.to_pyobject))])
...
let log_softmax ?axis ~x () =
   Py.Module.get_function_with_keywords __wrap_namespace "log_softmax"
     [||]
