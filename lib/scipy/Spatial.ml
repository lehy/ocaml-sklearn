...
let create ?radius ?center ?threshold ~points () =
   Py.Module.get_function_with_keywords __wrap_namespace "SphericalVoronoi"
     ...
     |> of_pyobject
let calculate_areas self =
   Py.Module.get_function_with_keywords (to_pyobject self) "calculate_areas"
     [||]
     []
...
| `T1_p_infinity x -> Wrap_utils.id x
                       ...
                       |> (fun py -> (Np.Obj.of_pyobject py : [`ArrayLike|`Ndarray|`Object] Np.Obj.t))
                  let geometric_slerp ?tol ~start ~end_ ~t () =
                     Py.Module.get_function_with_keywords __wrap_namespace "geometric_slerp"
                       [||]
                       (Wrap_utils.keyword_args [("tol", Wrap_utils.Option.map tol Py.Float.of_float); ("start", Some(start |> Np.Obj.to_pyobject)); ("end", Some(end_ |> Np.Obj.to_pyobject)); ("t", Some(t |> (function
| `F x -> Py.Float.of_float x
| `Ndarray x -> Np.Obj.to_pyobject x
)))])
