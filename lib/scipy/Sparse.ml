...
let asmatrix ?dtype ~data () =
   Py.Module.get_function_with_keywords __wrap_namespace "asmatrix"
     [||]
     (Wrap_utils.keyword_args [("dtype", dtype); ("data", Some(data ))])
...
| `Numpy_random_RandomState x -> Wrap_utils.id x
                       ...
                       |> (fun py -> (Np.Obj.of_pyobject py : [`ArrayLike|`Object|`Spmatrix] Np.Obj.t))
                  let rng_integers ?high ?size ?dtype ?endpoint ~gen ~low () =
                     Py.Module.get_function_with_keywords __wrap_namespace "rng_integers"
                       [||]
                       (Wrap_utils.keyword_args [("high", Wrap_utils.Option.map high (function
| `I x -> Py.Int.of_int x
| `Array_like_of_ints x -> Wrap_utils.id x
)); ("size", size); ("dtype", Wrap_utils.Option.map dtype (function
| `S x -> Py.String.of_string x
| `Dtype x -> Np.Dtype.to_pyobject x
)); ("endpoint", Wrap_utils.Option.map endpoint Py.Bool.of_bool); ("gen", Some(gen |> (function
| `PyObject x -> Wrap_utils.id x
| `None -> Py.none
))); ("low", Some(low |> (function
| `I x -> Py.Int.of_int x
| `Array_like_of_ints x -> Wrap_utils.id x
)))])
...
let prod iterable =
   Py.Module.get_function_with_keywords __wrap_namespace "prod"
     [||]
     (Wrap_utils.keyword_args [("iterable", Some(iterable ))])
