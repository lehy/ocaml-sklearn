...
let __iter__ self =
   Py.Module.get_function_with_keywords (to_pyobject self) "__iter__"
     ...
     []
        ...
        let next self =
           Py.Module.get_function_with_keywords (to_pyobject self) "next"
             [||]
             []
             |> (fun x -> (((fun py -> let len = Py.Sequence.length py in Array.init len
(fun i -> Py.Int.to_int (Py.Sequence.get_item py i))) (Py.Tuple.get x 0)), (Wrap_utils.id (Py.Tuple.get x 1))))
let to_string self = Py.Object.to_string (to_pyobject self)
let show self = to_string self
let pp formatter self = Format.fprintf formatter "%s" (show self)
...
module Dual = struct
let () = Wrap_utils.init ();;
let __wrap_namespace = Py.import "numpy.dual"

let get_py name = Py.Module.get __wrap_namespace name
let cholesky ?lower ?overwrite_a ?check_finite a =
   Py.Module.get_function_with_keywords __wrap_namespace "cholesky"
     [||]
     (Wrap_utils.keyword_args [("lower", Wrap_utils.Option.map lower Py.Bool.of_bool); ("overwrite_a", Wrap_utils.Option.map overwrite_a Py.Bool.of_bool); ("check_finite", Wrap_utils.Option.map check_finite Py.Bool.of_bool); ("a", Some(a |> Obj.to_pyobject))])
     |> (fun py -> (Obj.of_pyobject py : [`ArrayLike|`Ndarray|`Object] Obj.t))
let det ?overwrite_a ?check_finite a =
   Py.Module.get_function_with_keywords __wrap_namespace "det"
     [||]
     (Wrap_utils.keyword_args [("overwrite_a", Wrap_utils.Option.map overwrite_a Py.Bool.of_bool); ("check_finite", Wrap_utils.Option.map check_finite Py.Bool.of_bool); ("a", Some(a |> Obj.to_pyobject))])
...
let eigvals ?b ?overwrite_a ?check_finite ?homogeneous_eigvals a =
   Py.Module.get_function_with_keywords __wrap_namespace "eigvals"
     [||]
     (Wrap_utils.keyword_args [("b", Wrap_utils.Option.map b Obj.to_pyobject); ("overwrite_a", Wrap_utils.Option.map overwrite_a Py.Bool.of_bool); ("check_finite", Wrap_utils.Option.map check_finite Py.Bool.of_bool); ("homogeneous_eigvals", Wrap_utils.Option.map homogeneous_eigvals Py.Bool.of_bool); ("a", Some(a |> Obj.to_pyobject))])

let eigvalsh ?b ?lower ?overwrite_a ?overwrite_b ?turbo ?eigvals ?type_ ?check_finite a =
...
let fft ?n ?axis ?norm ?overwrite_x ?workers x =
   Py.Module.get_function_with_keywords __wrap_namespace "fft"
     [||]
     (Wrap_utils.keyword_args [("n", Wrap_utils.Option.map n Py.Int.of_int); ("axis", Wrap_utils.Option.map axis Py.Int.of_int); ("norm", Wrap_utils.Option.map norm Py.String.of_string); ("overwrite_x", Wrap_utils.Option.map overwrite_x Py.Bool.of_bool); ("workers", Wrap_utils.Option.map workers Py.Int.of_int); ("x", Some(x |> Obj.to_pyobject))])

let fft2 ?s ?axes ?norm ?overwrite_x ?workers x =
   Py.Module.get_function_with_keywords __wrap_namespace "fft2"
     [||]
     (Wrap_utils.keyword_args [("s", Wrap_utils.Option.map s (fun ml -> Py.List.of_list_map Py.Int.of_int ml)); ("axes", Wrap_utils.Option.map axes (fun ml -> Py.List.of_list_map Py.Int.of_int ml)); ("norm", Wrap_utils.Option.map norm Py.String.of_string); ("overwrite_x", Wrap_utils.Option.map overwrite_x Py.Bool.of_bool); ("workers", Wrap_utils.Option.map workers Py.Int.of_int); ("x", Some(x |> Obj.to_pyobject))])
...
let pinv ?cond ?rcond ?return_rank ?check_finite a =
   Py.Module.get_function_with_keywords __wrap_namespace "pinv"
     [||]
     (Wrap_utils.keyword_args [("cond", cond); ("rcond", rcond); ("return_rank", Wrap_utils.Option.map return_rank Py.Bool.of_bool); ("check_finite", Wrap_utils.Option.map check_finite Py.Bool.of_bool); ("a", Some(a |> Obj.to_pyobject))])
     |> (fun x -> (((fun py -> (Obj.of_pyobject py : [`ArrayLike|`Ndarray|`Object] Obj.t)) (Py.Tuple.get x 0)), (Py.Int.to_int (Py.Tuple.get x 1))))
let register_func ~name ~func () =
   Py.Module.get_function_with_keywords __wrap_namespace "register_func"
     [||]
     (Wrap_utils.keyword_args [("name", Some(name )); ("func", Some(func ))])

let restore_all () =
   Py.Module.get_function_with_keywords __wrap_namespace "restore_all"
     [||]
     []

let restore_func name =
   Py.Module.get_function_with_keywords __wrap_namespace "restore_func"
     [||]
     (Wrap_utils.keyword_args [("name", Some(name ))])

let solve ?sym_pos ?lower ?overwrite_a ?overwrite_b ?debug ?check_finite ?assume_a ?transposed ~b a =
   Py.Module.get_function_with_keywords __wrap_namespace "solve"
     [||]
     (Wrap_utils.keyword_args [("sym_pos", Wrap_utils.Option.map sym_pos Py.Bool.of_bool); ("lower", Wrap_utils.Option.map lower Py.Bool.of_bool); ("overwrite_a", Wrap_utils.Option.map overwrite_a Py.Bool.of_bool); ("overwrite_b", Wrap_utils.Option.map overwrite_b Py.Bool.of_bool); ("debug", debug); ("check_finite", Wrap_utils.Option.map check_finite Py.Bool.of_bool); ("assume_a", Wrap_utils.Option.map assume_a Py.String.of_string); ("transposed", Wrap_utils.Option.map transposed Py.Bool.of_bool); ("b", Some(b |> Obj.to_pyobject)); ("a", Some(a |> Obj.to_pyobject))])
     |> (fun py -> (Obj.of_pyobject py : [`ArrayLike|`Ndarray|`Object] Obj.t))
...
| `Stable -> Py.String.of_string "stable"
| `Mergesort -> Py.String.of_string "mergesort"
)); ("order", Wrap_utils.Option.map order Obj.to_pyobject); ("endwith", Wrap_utils.Option.map endwith Py.Bool.of_bool); ("fill_value", fill_value); ("a", Some(a |> Obj.to_pyobject))])
                       |> (fun py -> (Obj.of_pyobject py : [`ArrayLike|`Ndarray|`Object] Obj.t))
...
let create ?seed_seq () =
   Py.Module.get_function_with_keywords __wrap_namespace "PCG64"
     [||]
     (Wrap_utils.keyword_args [("seed_seq", seed_seq)])
     |> of_pyobject
...
| `S x -> Py.String.of_string x
                       ...
                       |> (fun py -> (Obj.of_pyobject py : [`ArrayLike|`Ndarray|`Object] Obj.t))
let broadcast_arrays ?kwargs args =
...
let fix ?out x =
   Py.Module.get_function_with_keywords __wrap_namespace "fix"
     [||]
     (Wrap_utils.keyword_args [("out", out); ("x", Some(x |> Obj.to_pyobject))])
     |> (fun py -> (Obj.of_pyobject py : [`ArrayLike|`Ndarray|`Object] Obj.t))
let flatnonzero a =
...
let fromfunction ?kwargs ~function_ shape =
   Py.Module.get_function_with_keywords __wrap_namespace "fromfunction"
     [||]
     (List.rev_append (Wrap_utils.keyword_args [("function", Some(function_ )); ("shape", Some(shape |> (fun ml -> Py.List.of_list_map Py.Int.of_int ml)))]) (match kwargs with None -> [] | Some x -> x))
...
let fromiter ?count ~iterable ~dtype () =
   Py.Module.get_function_with_keywords __wrap_namespace "fromiter"
     ...
     |> (fun py -> (Obj.of_pyobject py : [`ArrayLike|`Ndarray|`Object] Obj.t))
let frompyfunc ~func ~nin ~nout () =
   Py.Module.get_function_with_keywords __wrap_namespace "frompyfunc"
     [||]
     (Wrap_utils.keyword_args [("func", Some(func )); ("nin", Some(nin |> Py.Int.of_int)); ("nout", Some(nout |> Py.Int.of_int))])
...
let geterrcall () =
   Py.Module.get_function_with_keywords __wrap_namespace "geterrcall"
     ...
     []
     |> (fun py -> if Py.is_none py then None else Some (Wrap_utils.id py))
let gradient ?kwargs ~f varargs =
   Py.Module.get_function_with_keywords __wrap_namespace "gradient"
     (Array.of_list @@ List.concat [(List.map Wrap_utils.id varargs)])
     (List.rev_append (Wrap_utils.keyword_args [("f", Some(f |> Obj.to_pyobject))]) (match kwargs with None -> [] | Some x -> x))
...
| `S x -> Py.String.of_string x
                       ...
                       |> (fun py -> (Obj.of_pyobject py : [`ArrayLike|`Ndarray|`Object] Obj.t))
                  let set_printoptions ?precision ?threshold ?edgeitems ?linewidth ?suppress ?nanstr ?infstr ?formatter ?sign ?floatmode ?kwarg () =
                     Py.Module.get_function_with_keywords __wrap_namespace "set_printoptions"
                       [||]
                       (List.rev_append (Wrap_utils.keyword_args [("precision", Wrap_utils.Option.map precision Py.Int.of_int); ("threshold", Wrap_utils.Option.map threshold Py.Int.of_int); ("edgeitems", Wrap_utils.Option.map edgeitems Py.Int.of_int); ("linewidth", Wrap_utils.Option.map linewidth Py.Int.of_int); ("suppress", Wrap_utils.Option.map suppress Py.Bool.of_bool); ("nanstr", Wrap_utils.Option.map nanstr Py.String.of_string); ("infstr", Wrap_utils.Option.map infstr Py.String.of_string); ("formatter", formatter); ("sign", Wrap_utils.Option.map sign (function
| `Space -> Py.String.of_string " "
| `Plus -> Py.String.of_string "+"
| `Minus -> Py.String.of_string "-"
)); ("floatmode", Wrap_utils.Option.map floatmode Py.String.of_string)]) (match kwarg with None -> [] | Some x -> x))
